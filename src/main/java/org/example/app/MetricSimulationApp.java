...
public class MetricSimulationApp {
    ...
    // Dados Fixos para Cenários A e B
...
// O método selectScenario() precisa ser atualizado assim:
    private static void selectScenario(Scanner scanner, int metricChoice) {
        ...
        System.out.println("C. ENTRADA MANUAL (Apenas os dados necessários)");
        ...
        if (scenarioChoice.equals("A")) {
            ...
            // CHAMA O MÉTODO DE ENTRADA INTELIGENTE
        ...
        calculateAndReport(scanner, metricChoice, data);
    }
 ...
 private static ClassAnalysisData getRelevantManualInputData(Scanner scanner, int metricChoice) {
     ...
     // Valores padrão (se não forem solicitados, o cálculo usará estes)
     ...
     try {
         ...
         if (metricChoice == 2 || metricChoice == 6) {
             ...
             if (coupledInput.trim().isEmpty() && scanner.hasNextLine()) coupledInput = scanner.nextLine();
         ...
         if (metricChoice == 8) {
             ...
             if (childrenInput.trim().isEmpty() && scanner.hasNextLine()) childrenInput = scanner.nextLine();
    ...
    private static void calculateAndReport(Scanner scanner, int metricChoice, ClassAnalysisData data) {
